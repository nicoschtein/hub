...
class Hub {
  ...
  async proccess(id) {
    ...
    await Promise.all(balances.forEach(async (balance, i) => {
      ...
      if (balance !== 0) {
        ...
        const account = acountsToCheck[i];
        ...
        if (!burn || !toHell) {
          console.log(`Marking address ${address} as used failed while processing hub #${id}`);
        }
  ...
  async [sweep](id, sweeps) {
    ...
    while(sweeps.length) {
      ...
      if (transfer) {
        ...
        console.log(`Sweep for hub #${id} failed`);
      ...
      await Promise.all(batch.forEach(async (sweep) => {
        ...
        if (!saveSweep) {
          throw new Error(`Could not save sweep from address ${sweep.address} of account #${sweep.account} in hub #${id}`); 
        }
  ...
  async [credit](hubId, accountId, value) {
    ...
    if (!save) {
      console.log(`Failed to update balance of account #${accountId} of hub #${hubId} while crediting value ${value}`);
      return false;
    }
  ...
  async checkBalance(hubId, accountId, value) {
    ...
    const balance = await this.getBalance(hubId, accountId);
