...
class Hub extends EventEmitter {
  ...
  async process(id, scanUsed = false) {
    ...
    for (const balance of balances) {
      ...
      // Sweep only if address has balance and is not locked by another operation
      ...
      for (const confirmed of states) {
        ...
        if (confirmed) {
          ...
          const value = sweeps[i].value;
	        const index = sweeps[i].index;
          const address = sweeps[i].address;
          ...
          const destinationAddress = sweeps[i].destinationAddress;
  ...
  async [credit](hubId, accountId, value, offChain = false) {
    ...
    if (!save) {
      ...
      const balanceText = offChain ? '' : 'balanceOnTangle and ';
  ...
  credit(hubId, accountId, value) {
    ...
    return this[credit](hubId, accountId, value, true);
  ...
  async withdraw(hubId, accountId, {address, value, remainderAddress = null, checkAddress = true, tag = ''}) {
    ...
    let account = await this[store].get(constants.ACCOUNT_PREFIX, key);
    ...
    if (!sent) {
      ...
      account = await this[store].get(constants.ACCOUNT_PREFIX, key)
      account.credit += value;
      
      const restoreCredit = await this[store].set(constants.ACCOUNT_PREFIX, key, account)
  ...
  async [collectHotWalletInputs](value) {
    ...
    while (collectedValue < value) {
      ...
      if (!('address' in input) || !('value' in input) || !('keyIndex' in input)) {
	      console.log(`Skipped corrupted input: ${input}`);
        continue;
      }
      else if (!input.address || !Number.isInteger(input.value) || !Number.isInteger(input.keyIndex)) {
        console.log(`Skipped corrupted input: ${input}`);
	      continue;
      }
    ...
    for (const input in inputsToRestore) {
