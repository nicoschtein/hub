...
const replaySweep            = Symbol('replaySweep');
const checkViaDestination    = Symbol('checkViaDestination');
...
class Hub extends EventEmitter {
  ...
  dettach(id) {
    ...
    if (this[attachedHubs].indexOf(id) == -1) {
  ...
  async [sync](id, maxSweeps) {
    ...
    // Confirmed sweep indexes for each batch
    let confirmedSweepsIndexes = [];

    // List of confirmed sweeps
    const sweepsToCredit = [];

    // Delete confirmed sweeps transaction
    let deleteConfirmedSweeps = this[store].multi();
    ...
    while (hashesToBatch.length) {
      ...
      for (const confirmed of states) {
        ...
        const indexes = mapHashes[hash];

        // Process confirmed sweeps only in the end.
        if (confirmed) {
          
          // Push to confirmed sweeps list
          confirmedSweepsIndexes.push(indexes);

        }

        else {

          const first = sweeps[indexes[0]];
          const timestamp = first.t;

          const dt = Math.floor(Date.now() / 1000) - timestamp;

          // Check the destination address to mitigate against replay attacks
          if (dt > constants.REPLAY_SWEEPS_AFTER) {
          
            // Gather all inputs and value of sweeps in batch
            const inputs = [];
            const totalValue = 0;

            indexes.forEach((j) => {
              inputs.push(sweeps[j].address);
              totalValue += sweeps[j].value;
            });

            // Check if transaction confirmed via examining balance and related txs of destination address
            const confirmed = await this[checkViaDestination](destination, inputs, totalValue);
          
            if (confirmed) {
           
              // Push to confirmed sweeps list
              confrimedSweepsIndexes.push(indexes);

            }

            else { 

              // Skip replay sweeps that have not expired
              if ('rt' in first) {

                const drt = Math.floor(Date.now / 1000) - first.rt;
              
                if (drt < constants.REPLAY_SWEEPS_AFTER) {
                  continue;
                }
              ...
              // Execute replay
              const replay = await this[replaySweepTransfer](hash);
              ...
              if (replay) {
                ...
                // Update latest replay
                const updateReplay = this[store].multi();

                for (const j of indexes) {
              
                  sweeps[j].rt = replayTimestamp; 
             
                  updateReplay.lset(constants.PENDING_SWEEPS_PREFIX + id, j, sweeps[j]);

                }
         
                const updatedReplay = await new Promise((resolve) => {
                  updateReplay.exec((err, res) => {
                    if (err) {
                      console.log(err);
                      resolve(false);
                    }
                ...
                if (!updatedReplay) {
                  console.log(`Failed to update timestamp of ${indexes.length} pending sweeps after replaying in hub #${id}`);
                }
      ...
      for (const indexes of confirmedSweepsIndexes) {
        ...
        for (let j = 0; j < l; j++) {
          ...
          sweepsToCredit.push(sweep);

          const account = sweep.account;
          const value = sweep.value;
          const index = sweep.index;
          const address = sweep.address;
          
          const field = `${account}_${index}`;

          // Destination = hot wallet
          const destinationAddress = sweep.destinationAddress;

          // Release sweep's address, previously locked by process()
          // Prefix `0` indicates that the address is not locked
          deleteConfirmedSweeps.hset(constants.USED_ADDRESSES_PREFIX + id.toString(), field, `0_${address}`);  
	      
          // Commit release statement fot hotwallet address, previously locked by [sweep]()
          deleteConfirmedSweeps.decr(constants.HOT_WALLET_LOCKED_INPUTS_PREFIX + destinationAddress);

          // Remove pending sweep
          deleteConfirmedSweeps.lrem(constants.PENDING_SWEEPS_PREFIX + id.toString(), JSON.stringify(sweep)); 
    ...
    const deletedConfirmedSweeps = await new Promise((resolve) => {
      deleteConfirmedSweeps.exec((err, res) => {
        if (err) {
          console.log(err);
          resolve(false);
        }
    ...
    if (deletedConfirmedSweeps) {    

      for (const sweep of sweepsToCredit) {
      
        // Credit account with the value of the confirmed sweep
        const creditAccount = await this[credit](id, sweep.account, sweep.value);

        // Emit error event if account was not credited
        if (!creditAccount) {
          this.emit('creditError', {
            'id': id,
  ...
  // Check if sweep confirmed. (the anorthodox way)
  // Only works in the context of sweeps as implemented in Hub.
  // Will not work if one or more of the specific inputs were used more than once in transfers to destination address.
  async [checkViaDestination](destinationAddress, inputs, value) {

    // Get all transaction relate to the destinationAddres
    const txs = this[findTransactionObjects]({
      'addresses': [destinationAddress]
    });

    // Filter non-tail transactions
    txs.forEach((tx) => {
      return tx.currentIndex === 0;
    });

    // Extract hashes from tail transactions
    const hashes = [];
    ...
    // Fetch inclusion states in batches
    const batches = [];

    while (hashes.length > 0) {
      batches.push(hashes.splice(0, constants.INCLUSION_STATES_BATCH_SIZE));
    }
    ...
    for (const batch of batches) {
      ...
      const statesBatch = await this[getLatestInclusion](batch);

      states.concat(statesBatch);
    ...
    let inputsMatching = 0;
    let totalValue = 0;

    let confirmed = false;

    // For each transaction examine the bundle
    for (const tail of txs) {
    
      // Get bundle via tail hash
      const bundle = await getBundle(tail.hash);

      for (tx in bundle) {
        
        // Search for provided inputs and value
        if (tx.value < 0 && inputs.indexOf(tx.address) !== -1) {
        
          totalValue += tx.value;
          inputsMatching++;
          
          // Check if confirmation condition is valid
          if (totalValue === value && inputsMatching === inputs.length) {
            confirmed = true;
            break;
          }
      ...
      // Jump out the loop if confirmation was detected
      if (confirmed) {
        break;
      }
  ...
  [replaySweep](hash) {
    ...
    return new Promise((resolve) => {
      this[iota].api.replayBundle(hash, constants.DEPTH, constants.MWM, (err, res) => {
        if (err) {
          console.log(err);
          resolve(false);
        }
  ...
  getBundle(hash) {
    ...
    return new Promise((resolve) => {
      this[iota].api.getBundle(hash, (err, bundle) => {
        if (err) {
          console.log(err);
          resolve(false);
        }
  ...
  async [sweep](id, sweeps) {
    ...
    const hotWalletInput = await this[getNewHotWalletInput](true);
    ...
    while(sweeps.length) {
      ...
      if (transfer) {
        ...
        // Update expected balance of hotwallet input
  ...
  async getAccount(hubId, accountId) {
    ...
    const balanceOnTangle = await this[store].get(constants.ACCOUNT_PREFIX, `${key}_balanceOnTangle`);
    
    if (!balanceOnTangle) {
      console.log(`Failed to fetch balance on tangle of account #${accountId} in hub #${hubId}`);
      return false;
    }
    ...
    if (!credit) {
      console.log(`Failed to credit of account #${accountId} in hub #${hubId}`);
      return false;
    }
