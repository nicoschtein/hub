  ...
  async [sweep](id, sweeps) {
    ...
    while(sweeps.length) {
      ...
      await Promise.all(batch.forEach(async (sweep) => {
        ...
        const saveSweep = await this[store].rpush(constants[sweep_prefix], hubId, sweepObj);
        
        if (!saveSweep) {
          throw new Error(`Could not save sweep from address ${sweep.address} for account #${sweep.account}`); 
        }
